...
- **Assist layer (Phase 11):** LLM-powered features via Ollama (local, offline). DJ setlist analysis: paste a tracklist, LLM parses it into structured tracks, matches against local library, shows harmonic flow with key/BPM/energy arc. Settings panel with model selection and enable/disable toggle. Never on the critical analysis path — all features are user-initiated. Degrades gracefully when Ollama is not installed.
